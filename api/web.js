...
async function web (message) {
  try {
    ...
    if (_.includes(message, '&zone=')) {
      // console.log("++++++++++++++++ zone")
      const webArray = message.split('&')
      // console.log("++++++++++++++++ webArray", webArray)

      const domainArray = webArray[0].split('=')
      // console.log("++++++++++++++++ domainArray", domainArray)
      domain = domainArray[1]

      const zoneArray = webArray[1].split('=')
    ...
    if (_.includes(domain, 'http') || _.includes(domain, 'https')) {
      const url_domain = new URL(domain)
      const newDomain = (url_domain.hostname).split('www.')
    ...
    const configList = await checkConfig(domain, zone)
    ...
    if (configList.status === true && configList.type === 3) {
      ...
      const newFormat = await Promise.all((configList.data).map(list => formateData(domain, list)))
...
async function getConfigInfo (message, user_token) {
  try {
    ...
    const config = urlParams.get('submit')
    ...
    const resDB = await insertConfig(configInfo)
